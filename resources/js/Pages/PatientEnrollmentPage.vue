...
const submitOnly = async () => {
    ...
    try {
        ...
        if (response.status === 200) {
            ...
            alert.value = {
                    show: true,
                    message: 'Success! Patient enrolled.',
                    type: 'success',
                };
        ...
        if (error.response && error.response.data.errors) {
            // Check if duplicate error exists
            if (error.response.data.errors.duplicate) {
                //alert(error.response.data.errors.duplicate);
                form.errors['firstname'] = error.response.data.errors.duplicate[0];
            }

            // Assign other validation errors to the form
            Object.keys(error.response.data.errors).forEach(key => {
            form.errors[key] = error.response.data.errors[key][0]; // Only the first error message
        });
        } else {
        ...
        <form @submit.prevent="submit" class="form-container">
            ...
            <div>
                <InputLabel for="gender">
                    Gender <span class="red-asterisk">*</span>
                </InputLabel>
                <select id="gender" v-model="form.gender" class="mt-1 block w-full" @input="clearError('gender')">
            ...
            <div>
                <InputLabel for="marital_status">
                    Marital Status <span class="red-asterisk">*</span>
                </InputLabel>
