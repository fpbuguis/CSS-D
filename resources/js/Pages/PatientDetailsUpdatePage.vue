...
const fetchUserInfo = async (userId) => {
    try {
        ...
        console.log("User Details: ", selectedPatient.value)
    } catch (error) {
...
const fetchByUserId = async (userId) => {
    try {
        let response = await axios.get(`/api/patient-profile/${userId}`);
        patientDetails.value = response.data;
        patientId.value = patientDetails.value.patient_id;

        console.log("Patient ID:", patientId.value)

        return patientId.value;
    } catch (error) {
        console.error(`Failed to fetch patient with userId ${userId}:`, error);
    }
};

onMounted(async () => {
    await fetchUserInfo(userId.value);
    await fetchByUserId(userId.value);
});
    ...
    try {
        const patientData = {
            ...
            address: {
                ...
                address_brgy: form.address_brgy,
        ...
        const response = await axios.put(`/api/update-patient-info/${patientId.value}`, patientData);
        ...
        if (error.response && error.response.data.errors) {
            ...
            if (error.response.data.errors.duplicate) {
                alert.value = {
                    show: true,
                    message: error.response.data.errors.duplicate,
                    type: 'error',
                };
                return;
            }
