...
const fetchPatientByPatientId = async () => {
    try {
        let response = await axios.get(`/api/get-patient/${patientId}`);
        form.value.to = response.data.user_id
    } catch (error) {
        console.error(`Failed to fetch patient with patientId ${patientId}:`, error);
    }
}
...
const updateReceiverEmail = () => {
    ...
    // form.value.to = receiverEmail.value;
};
